...
func (pc *PaymasterController) GetUsageStats(c *gin.Context, apiKey string, address string, policyID int64) {
	...
	if err != nil {
		...
		types.SendError(c, nil, types.InternalErrorCode, "Failed to get policy")
		return
	}
	...
	if !ok {
		...
		types.SendError(c, nil, types.InternalErrorCode, "Invalid policy configuration")
		return
	}
	...
	if err != nil {
		...
		types.SendError(c, nil, types.InternalErrorCode, "Failed to get usage stats")
		return
	}
	...
	result := map[string]interface{}{
		...
		"usage": map[string]interface{}{
			"transactionCount": usageStats.TransactionCount,
			"gasUsed":          currentUsageEth.String(),
			"maxTransactions":  policy.Limits.MaxTransactionsPerWalletPerWindow,
			"maxGas":           policy.Limits.MaxEthPerWalletPerWindow,
		},
		"remaining": map[string]interface{}{
			"transactions": max(0, policy.Limits.MaxTransactionsPerWalletPerWindow-usageStats.TransactionCount),
			"gas":          remainingEth.String(),
		},
	}
	...
	if rateLimited {
		result["rateLimitReason"] = rateLimitReason
		if usageStats.EarliestTransactionTime != nil {
			result["earliestTransactionTime"] = usageStats.EarliestTransactionTime.Format(time.RFC3339)
		}
	}

	types.SendSuccess(c, nil, result)
}
