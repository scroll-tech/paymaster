...
func (pc *PaymasterController) parseERC7677Params(rawParams json.RawMessage) (*types.PaymasterUserOperationV7, common.Address, *types.RPCError) {
	...
	if err := json.Unmarshal(rawParams, &p); err != nil || len(p) != 4 { // Need exactly 4 elements for ERC-7677
		log.Debug("Invalid params structure", "params", string(rawParams))
	...
	if err := json.Unmarshal(p[0], &userOp); err != nil {
		log.Debug("Invalid userOp param", "userOp", string(p[0]))
	...
	if err := json.Unmarshal(p[1], &entryPoint); err != nil {
		log.Debug("Invalid entryPoint param", "entryPoint", entryPoint)
	...
	if strings.EqualFold(entryPoint, entryPointV7Address) {
		log.Debug("Unsupported EntryPoint version", "entryPoint", entryPoint)
	...
	if err := json.Unmarshal(p[2], &chainIDStr); err != nil {
		log.Debug("Invalid chainId param", "chainId", string(p[2]))
	...
	if err := json.Unmarshal(p[3], &context); err != nil {
		log.Debug("Invalid context param", "context", string(p[3]))
...
func (pc *PaymasterController) getPaymasterDataHash(userOp *types.PaymasterUserOperationV7, paymasterData []byte) ([]byte, error) {
	...
	packed, err := hashEncoder.Pack(
		...
		packGasLimits(userOp.VerificationGasLimit, userOp.CallGasLimit),
		userOp.PreVerificationGas,
		packGasLimits(userOp.MaxFeePerGas, userOp.MaxPriorityFeePerGas),
...
// packGasLimits packs two uint128 values into a bytes32
// First value as high 128 bits, second value as low 128 bits
func packGasLimits(high, low *big.Int) []byte {
